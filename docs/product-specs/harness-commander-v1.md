...
V1 的核心目标不是让 Harness 替代 Claude、Codex、Trae 等宿主工具，而是让 Harness 统一命令语义、输入约束、输出结构和产物落点；当前默认路径优先保证离线、可测试、可回归，只有明确允许的命令节点才可选接入宿主模型。
...
`init -> propose-plan -> plan-check -> 实现任务 -> sync -> distill -> check -> collect-evidence`
