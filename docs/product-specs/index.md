# 业务需求索引

## 这个文件是做什么的

用于汇总所有产品需求文档，帮助团队快速定位具体业务规则和命令级约束。

## 适合写什么

- PRD 文档目录
- 命令级产品/协议/测试/验收入口
- 当前生效版本
- 需求负责人或背景说明

## 推荐用法

- 把它作为产品知识库入口
- 新需求落地时，先登记到这里再细化文档
- AI 在写业务逻辑前，先从这里找到对应命令文档

## 当前主入口

- `Harness-Commander V1 命令导航`：`docs/product-specs/v1/index.md`

## 当前需求文档

- `init`：`docs/product-specs/v1/commands/init/product.md`
- `propose-plan`：`docs/product-specs/v1/commands/propose-plan/product.md`
- `plan-check`：`docs/product-specs/v1/commands/plan-check/product.md`
- `sync`：`docs/product-specs/v1/commands/sync/product.md`
- `distill`：`docs/product-specs/v1/commands/distill/product.md`
- `install-provider`：`docs/product-specs/v1/commands/install-provider/product.md`
- `check`：`docs/product-specs/v1/commands/check/product.md`
- `collect-evidence`：`docs/product-specs/v1/commands/collect-evidence/product.md`
- `run-agents`：`docs/product-specs/v1/commands/run-agents/product.md`
- `多宿主与多 agent 编排 V1`：`docs/product-specs/multi-host-orchestration-v1.md`
- `新用户引导`：`docs/product-specs/new-user-onboarding.md`
- `Init 模板规范`：`docs/design-docs/init-templates.md`
- `打包与 Skill 集成`：`docs/product-specs/packaging-as-skill.md`

## 历史与兼容入口

- `Harness-Commander 历史/兼容导航`：`docs/product-specs/harness-commander.md`
- `协议导航（薄入口）`：`docs/product-specs/harness-commander-command-protocol-v1.md`
- `验收导航（薄入口）`：`docs/product-specs/harness-commander-acceptance-v1.md`
- `测试导航（薄入口）`：`docs/product-specs/harness-commander-testing-v1.md`
