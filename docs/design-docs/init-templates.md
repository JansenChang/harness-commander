...
- [关键规则 1]: `harness init` 的运行时模板来源是 `src/harness_commander/init_templates/` 下的包内模板资源，本文件只负责描述模板规范与内容基线
...
- AC 1: `harness init` 执行时，对于每个缺失的文件，必须使用 `src/harness_commander/init_templates/` 中对应的包内模板资源进行创建
...
- 当模板规范文档本身缺失或损坏时，不影响 `harness init` 的运行时模板加载；但规范维护者仍需补齐本文件，确保包内模板资源与文档说明持续一致
