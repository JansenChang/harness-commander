...
- 问题 1：Phase 2 的默认入口是 `auto`、host-first，还是新的显式模式
  - 这是产品入口设计，而不是单纯参数解析问题
- 问题 2：provider 缺失时应直接 failure，还是允许回退到 heuristic
  - 这是产品/协议问题，不是模型调用细节
- 问题 3：宿主模型提炼成功但来源映射覆盖不足时，什么情况下是 success，什么情况下是 warning
  - 这决定下游 agent 如何消费参考材料可信度
...
- 明确默认模式是否从 `heuristic` 切到 `auto` 或新的 host-first 语义
...
- 让仓库能反映“Phase 1 已归档，Phase 2 产品规划已启动”。
